...
class GanttManager {
    ...
    setGanttChart() {
        ...
        ganttSketch = undefined;
        
        this.palette = [this.fontAndScaleClr,'#DC322F', '#2AA198', '#6C71C4','#859900', '#CB4B16'];  // Alternate syntax
        
        this.setGanttChartSketch();
        $('#gantt_chart').html(""); //empty out existing html
        ganttChart = new p5(ganttSketch, 'gantt_chart');
    }
    
    setGanttChartSketch() {
        ganttSketch = function(s) {
            ...
            let	borderClr =  style.getPropertyValue('--gantt-border-color');
            let fontAndScaleClr = style.getPropertyValue('--model-button-txt-color'); 
            let chunkInfoPaneBackground = style.getPropertyValue('--main-bg-color');
            let shadowClr = style.getPropertyValue('--gantt-text-shadow-color'); 
            ...
            s.preload = function() {
                ...
                if (G.darkLightManager.isDark) cpmLabel = s.loadImage('./images/CPM_Dark.png');
                else                           cpmLabel = s.loadImage('./images/CPM.png');
            }
