...
package classes {
	...
	public class FirstRun extends EventDispatcher{
		...
		private function itIsTheFirstRun():void {  
			...
			klmTxt = 		"*The Keystroke Level Model is a flat, serial list of all the interactions (operators) you need in order to accomplish some goal. In this example, we're modeling someone creating a new model in Cogulator.  As you probably know by now, we'd follow these steps to do that:"
							+ "\n" + "** 1st:  Click the '+' button to bring up the new model dialog"
							+ "\n" + "** 2nd: Enter a model name"
							+ "\n" + "**  3rd: Enter a collection name"
							+ "\n" + "* So, we'll create a model with three goals: for for each item in our list:"
							+ "\n"  
							+ "\n" + "** 1st:  Click the '+' button, then you enter the collection name"
							+ "\n" + "Look at 'New' button *  'Look'  assumes the user knows the location of the button.If the modeled user does not know the location, use 'Search'."
							+ "\n" + "Point to 'New' button"
							+ "\n" + "Click 'New' button"
							+ "\n"  
							+ "\n" + "** 2nd: Enter a model name"
							+ "\n" + "Look at 'Model Name' Field *Before typing, the user needs to look at the field"
							+ "\n" + "Think of model name *Think is used for generic mental operations - in the case, coming up with a name for the model"
							+ "\n" + "Hands to keyboard *Hands were on the mouse above.  In order to type, they need to be moved to the keyboard."
							+ "\n" + "Type KLM *Include exactly what's typed and only what's typed in the label (black text) after the 'Type' operator.  Cogulator uses that information to determine how long this step should take."
							+ "\n" + "Verify Correct *Typically a user will take a moment to verify what they've typed is correct before hitting the Enter key."
							+ "\n" + "Keystroke Tab key"
							+ "\n"  
							+ "\n" + "**  3rd: Enter a collection name"
							+ "\n" + "Look at 'Collection Name' Field"
							+ "\n" + "Think of collection name"
							+ "\n" + "Type Ex *Only have to enter Ex, the rest will autocomplete"
			...
			ngomslTxt =		"*Natural GOMS Language (NGOMSL) allows us to structure our task model around a series of goals to be accomplished to complete some task. In this example, we're modeling someone creating a new model in Cogulator.  As you probably know by now, we'd follow these steps to do that:"
							+ "\n" + "** 1st:  Click the '+' button to bring up the new model dialog"
							+ "\n" + "** 2nd: Enter a model name"
							+ "\n" + "**  3rd: Enter a collection name"
							+ "\n" + "* So, we'll create a model with three goals: for each item in our list:"
							+ "\n" 
							+ "\n" + "** 1st:  Click the '+' button, then you enter the collection name"
							...
							+ "\n" + ".Look at 'Model Name' Field *Before typing, the user needs to look at the field"
							+ "\n" + ".Think of model name *Think is used for generic mental operations - in the case, coming up with a name for the model"
							+ "\n" + ".Hands to keyboard *Hands were on the mouse in the previous goal.  In order to type, they need to be moved to the keyboard."
							+ "\n" + ".Type NGOMSL *Include exactly what's typed and only what's typed in the label (black text) after the 'Type' operator.  Cogulator uses that info to determine how long this step should take"
							+ "\n" + ".Verify Correct *Typically a user will take a moment to verify what they've typed is correct before hitting the Enter key."
							+ "\n" + ".Keystroke Tab key"
							+ "\n"  
							+ "\n" + "**  3rd: Enter a collection name"
							...
							+ "\n"  
							+ "\n" + "*The CPM-GOMS example models this same task. In it, you'll see a slightly more detailed version of the model and allowances for multitasking. Just a note, there's no reason you can't use multitasking in simpler, NGOMSL models. Check out the 'Listen to Lecture' goal in Sundry Methods in the Examples collection for an example of multitasking in a simpler task.";

			cpmTxt = 		"*In a CPM-GOMS-like, we're operating at the highest level of fidelity that Cogulator can provide. Of course, fidelity brings with it complexity. If you're not quite ready for CPM-GOMS, no big deal;  start with the KLM model, and work your way up from there. In this example, we're modeling someone creating a new model in Cogulator.  As you probably know by now, we'd follow these steps to do that:"
							+ "\n" + "** 1st:  Click the '+' button to bring up the new model dialog"
							+ "\n" + "** 2nd: Enter a model name"
							+ "\n" + "**  3rd: Enter a collection name"
							+ "\n" + "* So, we'll create a model with three goals: for each item in our list:"
							...
							+ "\n" + ".Also: Press the 'New' button as eyes"
							...
							+ "\n" + ". Initiate autocompleted text"
							+ "\n" + "* look at the field you'll be typing into"
							+ "\n" + ". Saccade to autocompleted text"
							+ "\n" + ". Look at autocompleted text (300 ms) *about 300ms per word proofreading speed"
							+ "\n" + ". Cognitive_processor Verify autocomplete"
							+ "\n" + ". Also: Attention to next field as eyes"
							...
							+ "\n" + ". Initiate Key Tab Key"
							+ "\n" + ". Keystroke Tab key"
							+ "\n" 
							+ "\n" + "Goal: Enter Model Name"
							+ "\n" + ". Think of Model Name"
							+ "\n" + ". Initiate Typing NGOMSL"
							+ "\n" + ". Type CPM-GOMS *after typing Ex the user sees the field has autocomplete"
							+ "\n" + ". Attend to Typed in Text"
							+ "\n" + ". Initiate Eye movement Typed in Text"
							+ "\n" + ". Saccade to Typed in Text"
							+ "\n" + ". Look at CPM-GOMS for (150 ms) * about 300ms to proofread a word, including saccade. So, we subtract out the saccade time of 150 ms, leaving us with 150 ms"
							+ "\n" + ". Cognitive_processor Verify Correct"
							+ "\n" + ". Initiate Key Enter Key"
							+ "\n" + ". Keystroke Enter key"
							+ "\n" 
							+ "\n" + "*For comparison, check out the NGOMSL model.  The task is identical, but we use higher level operators, which makes for a much simpler model. Of course, we lose some fidelity.  Depending on your modeling objectives that may or may not be a problem, and, of course, those sorts of questions are what should drive you to one level of detail or the other. As you build models with multitasking, remember that Cogulator is essentially just a calculator, not a genuine cognitive architecture. One of the ramifications of that is you'll need to check the Gantt chart and make sure the model is generating sensible results. For example, if a multitasked method is not completing before some dependent action in the main method, you'll need to move the multitasked method up a few lines.";
